...
import { handler } from "../../../backend/functions/message-sender/index";
import type { NotificationResult as HandlerResultItem } from "../../../backend/functions/message-sender/index";
import {
	getDbClient,
	closeDbClient,
...
import { createEventBridgeEvent } from "./utils/lambda-utils";
import { createTestUser, createAPODRecord } from "./utils/function-utils";
...
describe("Message Sender Lambda [integration]", () => {
	...
	// We only need one APOD record for the test, so we clear the table at the start and then create one record
	beforeAll(async () => {
		const dbUrl = process.env.DATABASE_URL_TEST as string;
		let setupClient: PoolClient | null = null;
		try {
			setupClient = await getDbClient(dbUrl);
			await setupClient.query("DELETE FROM NASA_APOD");
			await createAPODRecord();
		} finally {
			if (setupClient) {
				await closeDbClient(setupClient);
			}
		}
	});
