...
#   - Assumes US_with_timezone.sql is in the same directory as this script
# =================================================================
...
# SQL file to ingest
SQL_FILE="US_with_timezone.sql"
SQL_PATH="${SCRIPT_DIR}/${SQL_FILE}"

# Check if the SQL file exists
if [ ! -f "$SQL_PATH" ]; then
    echo "Error: SQL file not found at $SQL_PATH"
    echo "Please run add_timezone_to_cities.py first to generate the SQL file."
    exit 1
fi
...
echo "Ingesting cities data from $SQL_FILE into database..."
